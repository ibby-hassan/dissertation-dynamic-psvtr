...
/* ShapeRenderer takes voxelSpace (the App's internal state representing the shape
on the scene) and renders it as an object on the scene. */
/* voxelSet is basically an array where each index represents a space for a voxel
on the 2x2x2 grid. */
...
const ShapeRenderer = ({ voxelSpace }: ShapeRendererProps) => {
    ...
    const getPositionFromIndex = (index: number): [number, number, number] => {
        ...
        const z = index % 2;
    /* A voxel of size 1 placed at 0,0,0 will not sit on top of that point, but dead 
    in the centre of it. We're adding 0.5 to each coordinate to adjust it so it sits snapped
    onto the voxel space grid. This will need adjusted if the voxel size ever isn't 1x1x1. */
